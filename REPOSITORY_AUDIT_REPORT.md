...
**Amendment, 2026-08-24 (from the 2026-08-24 gate review's Finding 3):** this report's "native PostgreSQL 17 as a stand-in for the unverified `docker-compose.yml` path" framing (§0 below, §4.5, §4.6) was accurate on 2026-08-22 and is left as written — it is a record of what was true then, not a live claim. It stopped being true on two counts since: CI executed for real against `docker-compose.yml` on 2026-08-23/24 and passed end to end (`RISK_REGISTER.md` R-001, CLOSED), and `docker-compose.yml` now pins `postgres:17-alpine` (previously `16-alpine` — a version this report never mentions choosing and that no other document ever justified), matching the native install this report describes. Read this report as history; read `CLAUDE.md` and `RISK_REGISTER.md` for the current state.

**Second amendment, 2026-08-24 (from the second gate review's own Finding 3, `PHASE_1_GATE_REVIEW_2026-08-24-II.md`): widened to cover §3.2 as well.** The amendment above scoped its correction to §0/§4.5/§4.6 and missed §3.2's toolchain-summary table, which still stated "PostgreSQL 16" with no pointer to this note — the one specific stale fact the first amendment's own closure commit (`371e80a`) fixed everywhere else it appears. §3.2's table row now carries an inline correction pointing back here rather than being silently left wrong.
...
**Environment note:** Docker is not available in this session's sandbox (`docker`/`docker compose` — command not found, in both Bash and PowerShell). `docker-compose.yml` is provided and is what CI (`.github/workflows/conformance.yml`) and any developer with Docker will use. To still prove the live-DB mechanism actually works rather than just compiling, this session used a PostgreSQL 17 instance already installed natively on this machine (found listening on `localhost:5432`) as a substitute — first a single scratch role (section 4.5), later replaced with the proper `nexora_migrate`/`nexora_app` role pair (section 4.6) once the RLS-bypass finding required it. Both roles and the database remain in place, harmless and dev-only, matching what `docker-compose.yml` + `platform/db/init/001_roles.sql` would produce. Confirmed by the user 2026-08-22: no Docker install planned yet, so the Docker path itself (vs. this native-Postgres stand-in) remains unverified — tracked as `RISK_REGISTER.md` R-001, deliberately kept open.
...
| Database setup | MATCH | `docker-compose.yml` provides PostgreSQL 16 on host port 5433 (chosen to avoid colliding with a developer's pre-existing local Postgres — see `DECISION_LOG.md`). **Stale as of 2026-08-24 (gate review Finding 3, missed by this document's own §0/§4.5/§4.6 amendment note above): `docker-compose.yml` now pins `postgres:17-alpine`, not 16 — see the amendment note at the top of this file.** `platform/config.ts` loads `DATABASE_URL` from the environment with a dev-friendly default matching docker-compose. `platform/db/kysely.ts`'s `createDb()` is the one place a Kysely instance is constructed, wrapping `platform/db/pool.ts`'s `pg.Pool`. |
| Migration runner + RLS/transaction helper | MATCH | `platform/db/migrate.ts` (forward-only, raw SQL, tracked in `schema_migrations`, runs as `nexora_migrate`) + `platform/db/discover-migrations.ts` (discovers `modules/<module>/migrations/*.sql`) + `platform/db/tenant-context.ts` (the one `@singleton-role: tenant-context` helper, on Kysely, runs as `nexora_app` — opens a `db.transaction()`, sets `app.tenant_id` via `set_config(..., true)` as the first statement, commits/rolls back). Proven against a real, running PostgreSQL both structurally (section 4.5) and behaviorally (`platform/db/tenant-context.spec.ts`: tenant isolation, fail-closed, no cross-call leakage, rollback-on-error, plus a safety assertion that the connected role cannot bypass RLS — 5/5 passing). Two roles + `FORCE ROW LEVEL SECURITY` + the runtime safety assertion are now all implemented — see `DECISION_LOG.md` "RLS: FORCE ROW LEVEL SECURITY or a non-owner app role" (RESOLVED). A conformance rule (`tools/conformance/rules/db-access.ts`) mechanically forbids any other file from importing `pg` directly or calling `.transaction(`. |
...
| Tests | MISSING (product) / MATCH (harness + DB helper) | No product tests exist (no product code exists). Three self-test files, 31/31 passing: `tools/conformance/harness.selftest.spec.ts` (20 cases, static rules incl. `db-access.ts` and `SCHEMA-MISSING-FORCE-RLS`), `tools/conformance/harness.selftest.live-db.spec.ts` (6 cases, schema rules against a real migrated PostgreSQL), `platform/db/tenant-context.spec.ts` (5 cases: the RLS helper's behavior against real PostgreSQL, plus the role-safety assertion). |
...
- `tools/conformance/rules/schema.ts` — static SQL-text check: parses `**/migrations/*.sql`: every non-exempt table must have `tenant_id`; every non-exempt table must have both `ENABLE ROW LEVEL SECURITY` and a `CREATE POLICY`; every table with those two must also have `FORCE ROW LEVEL SECURITY` (added this session — without it, the table's owning role bypasses RLS entirely, see `DECISION_LOG.md`); no `FLOAT`/`DOUBLE`/`REAL` column on a money-shaped column name; no more than one `*idempotency*`-named table across all migrations.
...
- `tools/conformance/fixtures/**` — 18 deliberately-violating fixtures (one or more per rule) plus two control fixtures: `clean/` (must produce zero violations across every static rule) and `db-access-allowed-platform-files/` (must produce zero violations for the files legitimately allow-listed to touch `pg`/`.transaction(`).
- `tools/conformance/harness.selftest.spec.ts` — 20 Vitest cases: one asserting detection per fixture, plus two asserting the control fixtures are silent.
- `tools/conformance/harness.selftest.live-db.spec.ts` — 6 Vitest cases proving the schema rules against a real PostgreSQL connection (5 violation fixtures, incl. `SCHEMA-MISSING-FORCE-RLS`, + the clean control), isolated in a dedicated schema that's reset between fixtures. Uses the migrate role (schema-structure check, not a tenant-isolation proof).
- `platform/db/tenant-context.spec.ts` — 5 Vitest cases proving the RLS helper itself (not just the schema rules) against real PostgreSQL: the app role cannot bypass RLS (asserted first, via `platform/db/assert-role-safety.ts`), tenant isolation, fail-closed with no context, no leakage across pooled-connection reuse, rollback-on-error. Uncovered a real finding, since resolved — see `DECISION_LOG.md` "RLS: FORCE ROW LEVEL SECURITY or a non-owner app role" and `RISK_REGISTER.md` R-002.
...
npx vitest run                      -> 31/31 self-test cases pass (20 static + 6 live-DB schema + 5 tenant-context)
...
**Superseded by section 4.6:** the single scratch `nexora` role/database described above was later dropped and replaced with the `nexora_migrate`/`nexora_app` role pair once the RLS-bypass finding below was resolved.

### 4.6 Role separation and RLS-bypass safety net (third pass, this session)

While writing `platform/db/tenant-context.spec.ts`, empirically confirmed (via `psql`) that a table's owning role bypasses `ENABLE ROW LEVEL SECURITY` by default — with the single `nexora` role from 4.5 also owning every table it migrated, it would have silently bypassed RLS the moment real tenant-owned tables existed. The user directed a three-layer fix, all implemented and verified against the local PostgreSQL 17 instance (see `DECISION_LOG.md` "RLS: FORCE ROW LEVEL SECURITY or a non-owner app role", `RISK_REGISTER.md` R-002):

```
On the local Postgres: dropped the old `nexora` database/role, created
nexora_migrate (owns the schema) and nexora_app (NOSUPERUSER NOBYPASSRLS,
never an owner), with ALTER DEFAULT PRIVILEGES so nexora_app automatically
gets DML rights on every future table nexora_migrate creates. The same
statements are captured in platform/db/init/001_roles.sql, mounted into
docker-compose.yml's postgres service at /docker-entrypoint-initdb.d/.

npx vitest run (DATABASE_URL=...nexora_app..., MIGRATE_DATABASE_URL=...nexora_migrate...)
  -> 31/31 pass, including platform/db/tenant-context.spec.ts's new safety-assertion
     test and two new SCHEMA-MISSING-FORCE-RLS self-test cases (static + live).

Misconfiguration smoke tests (each run in isolation, then reverted):
  -> DATABASE_URL pointed at nexora_migrate (the owner): tenant-context.spec.ts
     failed immediately with "Connected role 'nexora_migrate' owns table
     'tenant_context_selftest'...", 0 of the isolation tests ran.
  -> DATABASE_URL pointed at the postgres superuser: failed immediately with
     "Connected role 'postgres' is a PostgreSQL superuser...", same effect.
  Both confirm the safety assertion actually blocks a misconfigured role from
  producing a false-green tenant-isolation result, not just that it compiles.

Smoke test of the new schema rule: a throwaway table with tenant_id + ENABLE +
policy but no FORCE was flagged as SCHEMA-MISSING-FORCE-RLS by both the static
and live checkers (2 violations, same pattern as the 4.5 smoke test); cleaned
up afterward, scan re-run clean.
```

The `nexora_migrate`/`nexora_app` roles and the `nexora` database remain in place on the local Postgres instance (harmless, dev-only) for a future session to reuse.

---

## 5. Actions taken this session (for the record)

- `npm install` — dependencies `pg`, `kysely`; dev dependencies `typescript`, `tsx`, `vitest`, `@types/node`, `@types/pg`.
- `git init`, with `git config --local user.email` set to the user's own address (the machine's *global* git email was found to be a leftover placeholder string, not a real address — flagged to the user, who chose to fix it locally only; global config was not touched, per instruction never to modify it). Commits made across this session's three passes; each stopped for review before the next began, per this report's own gate.
- No git remote added and nothing pushed — explicit user instruction, repo is local-only for now.
- On the local, pre-existing PostgreSQL 17 instance on this machine (unrelated to this project): created a `nexora` role/database as a stand-in for `docker-compose.yml`'s Postgres since Docker isn't available here (section 4.5), later dropped and replaced with the `nexora_migrate`/`nexora_app` role pair once the RLS-bypass finding was resolved (section 4.6). Both roles and the `nexora` database remain in place, dev-only, not referenced by anything outside this repo's own config defaults.
...
6. ~~`FORCE ROW LEVEL SECURITY` vs. a non-owner application role~~ — **RESOLVED** 2026-08-22: both, plus a runtime assertion. See `DECISION_LOG.md` "RLS: FORCE ROW LEVEL SECURITY or a non-owner app role" and `RISK_REGISTER.md` R-002.
