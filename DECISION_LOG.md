...
**Current state: (A) remains implemented for Task 1**, which is sufficient because `store.read`'s failure branch is unreachable (a `store_memberships` row implies the store exists through its foreign key), so no failure audit can currently be lost. **(B) becomes mandatory at Task 2**, whose first capabilities audit reachable failures (`store.create` on a reserved slug, `membership.invite` on a duplicate). Implementing it belongs in the Task 2 change that first needs it, not retrofitted now — building the second-connection machinery with no reachable failure to exercise it would ship untested infrastructure.
