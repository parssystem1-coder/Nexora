...
**Recommendation:** B (or C). A separate low-privilege app role is the safer default for defense in depth beyond RLS alone, and doesn't rely on every future migration remembering to add `FORCE`. `FORCE ROW LEVEL SECURITY` (A) is the pragmatic fix used in `tenant-context.spec.ts`'s own scratch table this session, since Phase 0 has only one role and adding a second role/credential is Task-1-scope role/credential design, not a Phase 0 scaffold change.
**Status:** OPEN — needs a decision before Task 1's real migrations start creating tenant-owned tables; whichever is chosen should probably become a mechanical conformance rule (schema.ts / schema-live.ts currently only check that `ENABLE ROW LEVEL SECURITY` + a policy exist, not `FORCE` or role ownership) rather than left to migration-review discipline alone.
