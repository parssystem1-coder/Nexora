...
**Decision:** `roles`, `permissions`, `role_permissions` are platform-global reference data (no `tenant_id`, no RLS — added to `TENANT_EXEMPT` alongside the identity cluster), seeded with a fixed Phase 1 catalog (`owner`/`admin`/`member` roles, a `store.read` permission, all three roles granted it). `membership_roles` (which membership — a specific user-in-an-organization — holds which role) is the only table in this group that is genuinely tenant-scoped, since a membership only exists within one organization.
**Status:** RESOLVED. Revisit `roles`/`permissions` exemption if/when tenant-custom roles become an actual Phase 2+ feature.
...
**Recommendation / Decision:** B. Implemented by extending `TENANT_EXEMPT` in both `tools/conformance/rules/schema.ts` and `schema-live.ts` to `{users, currencies, reserved_subdomains, sessions, credentials, identity_providers, schema_migrations}`.
**Status:** OPEN — needs human confirmation that this reading of §5 is correct (recommend also fixing the sentence in `08_PHASE_1_BRIEF.md` §5 itself to name the exemption as "the identity cluster," not enumerate `users` alone, so this doesn't have to be rediscovered per-table). Proceeding under B for Task 1 since the alternative is not implementable.
