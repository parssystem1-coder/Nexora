...
**Correction, 2026-08-28 (first pass): the "Actions quota/plan" explanation first assumed here does not hold up.** Settings → Billing and licensing → Usage was checked: month-to-date gross metered usage is ~$7 with **$0.00 billed** (fully offset by discount — no spending threshold crossed), and its usage curve rises well before this repository even had a git remote (added 2026-08-23), so most of that $7 isn't this repository's CI. Against GitHub Free's ~2,000 included private-repo Actions minutes (a derived estimate, not a page reading), ~$7 of mostly-unattributed usage does not look like an exhausted allowance. The quota/plan theory was withdrawn at that point with no replacement cause asserted.

**Correction, 2026-08-28 (second pass): the cause is now IDENTIFIED, not unknown.** Settings → Billing and licensing → Budgets and alerts shows five account-scoped budgets, one on Product: Actions, each **$0** with "Stop usage when budget limit is reached" = Yes — 100% by construction, no threshold to cross. Public repositories don't meter Actions minutes at all, so nothing is ever charged against this budget; private repositories do meter them (even fully within the free allowance), so the budget blocks scheduling outright — exactly the observed queued/zero-jobs symptom. The budget cannot currently be edited or deleted ("A valid payment method is required" on both operations) — a known issue affecting other accounts too (github.com/orgs/community/discussions/178030). Full record: `DECISION_LOG.md` 2026-08-27/28.

**The earlier "may break CI again" hedge is superseded, not merely restated: this is not a possibility, it is a certainty while the budget stands.** Making this repository private again disables CI entirely and indefinitely — not until a reset (there is nothing to reset; the budget is zero, not exhausted), but until GitHub Support removes/raises the budget, a payment method is added, or the repository moves to a separately-billed organization (GitHub Education's free Team tier for verified teachers is being pursued). A self-hosted runner would sidestep this but is not built. The repository stays public on that basis, not as a temporary wait.
...
- **R-008** (OPEN, intermittent) — see §4 above and `RISK_REGISTER.md` for the full record.
- **R-009** (new, OPEN, structural) — all ten controllers call `recordAuditEventDurable` outside the `try/catch` wrapping the domain transaction (the golden path's own shape, inherited by every slice). A throwing audit write after a successful commit can surface to the client as a failure for an operation that actually succeeded. Verified by reading all ten: nine are protected either by a unique constraint that turns a client retry into a clean `CONFLICT` (`organization.create`, `membership.invite`, `membership.role.assign`, `store.create`, `membership.revoke`), by the guard chain blocking a retry outright (`auth.logout`, `auth.logout_all`), or by the write being naturally idempotent (`organization.switch`); `auth.login` is genuinely exposed — a retry can create a second, orphaned ACTIVE session row never handed to any client. Low likelihood, low-to-real impact depending on capability. See `RISK_REGISTER.md` R-009 for the full analysis, including that this is a silence in ADR-034 rather than a contradiction of it, and candidate directions (none chosen).
