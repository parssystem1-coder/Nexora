...
- **Amendment, 2026-09-03 (sixth to this file) — the unit a provider speaks, and the granularity it can charge (ADR-023's dated amendment; ADR-055's second).** **No table is added by this amendment, and §4's list stays at 31** — said explicitly because a reader who sees a direct-debit mandate discussed in ADR-058 will look for one, and there is none: ADR-058 rules direct debit out of V1 and creates nothing. Three constraints, all on **item 12**: the port declares a **wire unit** (the divisor between stored Rials and the unit the provider's protocol carries) and **the adapter is the only place conversion happens** — Application and Domain see `Money` in stored units only, the same fence ADR-023 item 9 draws around SDKs and field names; **verify compares in stored units**, so ADR-023 item 5's comparison is always Rial against Rial; and the port declares a **charging granularity**, with an off-granularity amount arriving at an adapter being a **hard failure, never a silent rounding**. **The matching constraint on item 13** is ADR-055's: `tax_amount_minor` is chosen as the value nearest the nominal tax such that `subtotal_minor + tax_amount_minor` lands on that granularity, so the invoice identity holds exactly and the gateway can charge the total. **Two invoices with the same subtotal and rate can therefore carry different tax through providers of different granularity** — which is why those columns are snapshotted per invoice rather than recomputed.
